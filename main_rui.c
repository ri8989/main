...
//	rcl_publish(&publisher, &msg, NULL);
	...
	pub_ext_msg.data = can_6;
	rcl_publish(&publisher_ext, &pub_ext_msg, NULL);
}
...
void step_sequence(void) {
    ...
    if (time < 1800 && flag_h != 0) {
    	...
    	else
    	{
             ...
             else if(can_13 > 12000)
             {
            	 order_data_4[0] = -4500;
             }
    		//order_data_4[0] = -4000;
    	}
//        if (can_12 < -1000 && can_6 > -190000) {
//            order_data_3[3] = 5500;
//        }
//        else{
//        	flag_h = 0;
...
//        if (can_11 <= -20000) {
//            order_data_3[2] = 1000;
//        } else if (can_11 >= -10000) {
//            order_data_3[2] = -1000;
//        }
//                if (can_11 <= -78000) {
//                    order_data_3[2] = 700;
//                } else if (can_11 >= -70000) {
//                    order_data_3[2] = -700;
//                }
	    if (can_6 < -260000) {
	        order_data_2[0] = 0;
	        order_data_2[1] = 0;
	    } else if (can_6 > -250000) {
	        order_data_2[0] = 1550;
	        order_data_2[1] = -1550;
	        //up
	    }
	    //if(flag_h == 0) order_data_4[0] = -1900;
    }
////    // Step (3),(8) time 500〜
//    else if (time < 4000 || flag_h == 0) {
//    	//flag_h = 0;
//    	order_data_4[0] = -1550;
//    	//order_data_3[3] = -2800;
//        // (3)move hanasu
...
void rock()
{
    ...
    if (time_2 < 500) {
       ...
       __HAL_TIM_SET_COMPARE(&htim2,TIM_CHANNEL_2,125);
   ...
   //    // Step (3),(8) time 500〜
          ...
          //__HAL_TIM_SET_COMPARE(&htim2,TIM_CHANNEL_4,175);
           }
       else{
    	   order_data_3[0] = 0;
...
void fear()
{
    ...
    else if(ps5.triangle_btn == 1)
    {
    	 __HAL_TIM_SET_COMPARE(&htim2,TIM_CHANNEL_1,210);
        __HAL_TIM_SET_COMPARE(&htim2,TIM_CHANNEL_2,210);
        __HAL_TIM_SET_COMPARE(&htim2,TIM_CHANNEL_4,210);
        //__HAL_TIM_SET_COMPARE(&htim4,TIM_CHANNEL_2,175);
    }
    else{
    	 __HAL_TIM_SET_COMPARE(&htim2,TIM_CHANNEL_1,0);
        __HAL_TIM_SET_COMPARE(&htim2,TIM_CHANNEL_2,0);
...
void can_train3()
{
    ...
    if (fabs(ps5.joy_right_x > 0.1)){
    	order_data[0] = -2000;
    }
    else if (ps5.joy_left_x > 0.1){
    	order_data[1] = 5000*ps5.joy_left_x;
    }
    else if (ps5.joy_left_x < 0.1){
    	order_data[1] = 5000*ps5.joy_left_x;
    }
...
void shot2()
{
    ...
    if(ps5.circle_btn == 1)
    {
        ...
        else if(plus + 5000 >= 25000)
        {
            plus = 0;
        }

        order_data[0] = 5000+plus;
        order_data[1] = 5000+plus;
        order_data[2] = -5000-plus;
        order_data[3] = -5000-plus;
    }
    else if(ps5.cross_btn == 1)
    {
        order_data[0] = 5000;
...
void crazy_crazy()
{
    ...
    else if (ps5.down_btn == 1){
    	order_data_3[0] = -1500;
    }
    else if (ps5.left_btn == 1){
    	order_data_3[1] = 1500;
    }
    else if (ps5.right_btn == 1){
    	order_data_3[1] = -1500;
    }
    else if (ps5.L1_btn == 1){
    	order_data_3[2] = 1500;
    }
    else if (ps5.R1_btn == 1){
    	order_data_3[2] = -1500;
...
void ball_c()
{
	if(ps5.Joy_click_R == 1)
	{
		flag_h = 2;
	}
	else if(ps5.R2_btn == 1)
	{
		flag_h = 1;
	}
	else if(ps5.L2_btn == 1)
	{
		flag_h = 0;
		//order_data_3[3] = -2500;
	}
...
void ball_d()
{
	...
	if(ps5.right_btn == 1)
	{
		order_data_3[0] = -16000;
	}
	else if(ps5.left_btn == 1)
	{
		order_data_3[0] = 3000;
	}
//	else if(ps5.square_btn == 1)
//	{
//		order_data_4[0] = -10000;
//	}
	else
	{
		//order_data_4[0] = -1000;
...
void can_up()
{
    ...
    else if(ps5.Joy_click_L == 1)
    {
    	order_data_3[2] = -1500;
    }
    else if(ps5.Joy_click_R == 1)
    {
    	order_data_3[3] = 2000;
    }
    else if(ps5.PS_btn == 1)
    {
    	order_data_3[3] = -2000;
    }
//    else if(ps5.L2_btn == 1)
...
//int16_t get_speed(float input) {
//    if (fabs(input) < DEADZONE) return 0;
//    else if (fabs(input) < 0.5) return (input > 0 ? 1000 : -1000);
//    else if (fabs(input) < 0.8) return (input > 0 ? 3000 : -3000);
//    else return (input > 0 ? 5000 : -5000);
//}

int16_t get_speed(float input) {
    if (fabs(input) < DEADZONE) return 0;

    return (int16_t)(input * 5000);  // 入力を直接スケーリング（柔らかくて滑らか）
}

void omni_control() {
    int16_t vx = get_speed(ps5.joy_left_y);  // 前後
    int16_t vy = get_speed(ps5.joy_left_x);  // 左右

    if(ps5.L1_btn == 1)
    {
        order_data[3] = 2000;  // 右下 (1)
        order_data[2] = 2000;  // 右上 (2)
        order_data[1] = 2000; // 左下 (3)
        order_data[0] = 2000;
    }
    else if(ps5.R1_btn == 1)
    {
    ...
    // ロボマスター順番：左上4, 左下3, 右上2, 右下1
    // 回転なしの平行移動（+X = 右, +Y = 前）を以下のベクトルで分解
    order_data[3] = vy - vx;  // 右下 (1)
    order_data[2] = vy + vx;  // 右上 (2)
    order_data[1] = -vy - vx; // 左下 (3)
    order_data[0] = -vy + vx; // 左上 (4)
    }
}
...
int16_t pid_control(PID_t *pid, float target, float actual) {
...
//
//void movement() {
//    float x = ps5.joy_left_x;
//    float y = ps5.joy_left_y;
//    float threshold = 0.2f;
//
//    // 入力が小さいときは止まる
//    if (fabs(x) < threshold && fabs(y) < threshold && ps5.R1_btn == 0) {
//        for (int i = 0; i < 4; i++) order_data[i] = 0;
//        return;
//    }
//
//    // 各輪の目標速度（オムニ4輪）
...
//    } else {
//        // 通常移動（オムニ機構に応じた式）
//        float vx = x * 1000;  // 横方向速度
//        float vy = y * 1000;  // 縦方向速度
//
//        // モーター配置：
//        // 左上(4): +vx +vy
//        // 左下(3): -vx +vy
...
//        // 右下(1): -vx -vy
//
//        target_speed[0] = +vx + vy;  // 左上 (ID 0x204)
//        target_speed[1] = -vx + vy;  // 左下 (ID 0x203)
//        target_speed[2] = +vx - vy;  // 右上 (ID 0x202)
//        target_speed[3] = -vx - vy;  // 右下 (ID 0x201)
//    }
//
//    // 現在の速度
...
//    // PID制御で電流指令に変換
//    for (int i = 0; i < 4; i++) {
//        order_data[i] = pid_control(&pid[i], target_speed[i], actual_speed[i]);
//    }
//}
...
void StartTask02(void *argument)
{
  ...
  for(;;)
  {
	  //encoder_count_1 = Encoder_readCount(&encoder);
	  //encoder_count_2 = Encoder_readCount(&encoder_2);
